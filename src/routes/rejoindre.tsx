...
import { ArrowDown, BadgeCheck, BriefcaseBusiness, Check, ClipboardCheck, FileCheck2, FileText, Handshake, MessageSquareText, ShieldCheck, TimerReset, UserRoundCheck, WalletCards, X } from "lucide-react";
...
function Join() {
  return <>
    ...
    <section className="py-20 md:py-24"><div className="section-shell"><div className="max-w-3xl"><p className="eyebrow">Un modèle clair, sans marketplace</p><h2 className="mt-4 text-3xl font-bold md:text-4xl">Les responsabilités FORMA restent protégées.</h2><p className="mt-4 leading-7 text-muted-foreground">FORMA pilote directement ses métiers stratégiques. Le réseau intervient uniquement sur les expertises complémentaires sélectionnées et encadrées.</p></div><div className="mt-10 grid gap-5 lg:grid-cols-2"><article className="border-t-4 border-primary bg-card p-6 shadow-soft md:p-8"><ShieldCheck className="h-7 w-7 text-accent" /><p className="eyebrow mt-8">Pilotage interne direct · non sous-traité</p><h3 className="mt-3 text-2xl font-bold">Activités stratégiques FORMA</h3><ul className="mt-6 grid gap-3 text-sm font-semibold">{["Sécurité privée", "Gardiennage & protection de sites", "Coordination événementielle", "Couverture média & relation client"].map((item) => <li key={item} className="flex gap-3"><Check className="mt-0.5 h-4 w-4 shrink-0 text-accent" />{item}</li>)}</ul></article><article className="border-t-4 border-accent bg-card p-6 shadow-soft md:p-8"><Handshake className="h-7 w-7 text-accent" /><p className="eyebrow mt-8">Expertises complémentaires coordonnées</p><h3 className="mt-3 text-2xl font-bold">Réseau de partenaires professionnels</h3><ul className="mt-6 grid gap-3 text-sm font-semibold">{["Maintenance technique", "Services opérationnels", "Logistique", "Prestations événementielles spécialisées"].map((item) => <li key={item} className="flex gap-3"><Check className="mt-0.5 h-4 w-4 shrink-0 text-accent" />{item}</li>)}</ul><p className="mt-6 flex gap-3 border-t border-border pt-5 text-sm font-bold text-destructive"><X className="mt-0.5 h-4 w-4 shrink-0" />La sécurité et le gardiennage ne sont jamais proposés comme catégories de sous-traitance.</p></article></div></div></section>
