  ...
  return <div className="min-h-screen bg-background text-foreground">
    <header className="sticky top-0 z-50 border-b border-border/60 bg-primary/95 text-primary-foreground backdrop-blur-xl">
      <div className="section-shell flex h-20 items-center gap-5">
        ...
        <nav className="hidden items-center gap-4 xl:flex" aria-label="Navigation principale">
          ...
          <Link to="/connexion-client" activeProps={{ className: "text-accent" }} className="border-l border-primary-foreground/20 pl-4 text-xs font-semibold text-primary-foreground/70 transition-colors hover:text-primary-foreground 2xl:hidden">Espace Client</Link>
          <Link to="/rejoindre" activeProps={{ className: "text-accent" }} className="text-xs font-semibold text-primary-foreground/70 transition-colors hover:text-primary-foreground 2xl:hidden">Prestataire</Link>
        </nav>
      ...
      {open && <nav className="max-h-[80vh] overflow-y-auto border-t border-primary-foreground/10 bg-primary px-5 py-5 xl:hidden" aria-label="Navigation mobile">
        ...
        <p className="mt-5 text-[10px] font-bold uppercase tracking-wider text-primary-foreground/45">Réseau de partenaires professionnels</p>
        <Link to="/rejoindre" onClick={() => setOpen(false)} className="mt-2 flex items-center gap-2 border-b border-primary-foreground/10 pb-3 text-sm font-medium"><Users className="h-4 w-4 text-accent" />Candidater · Découvrir l’espace partenaire</Link>
    ...
    <footer className="bg-primary pb-24 text-primary-foreground md:pb-0">
      <div className="section-shell grid gap-10 py-14 md:grid-cols-[1.3fr_1fr_1fr]">
        ...
        <div><p className="eyebrow">Entreprise</p><div className="mt-4 grid gap-3 text-sm"><Link to="/a-propos">À propos</Link><Link to="/devis">Demander un devis (clients)</Link><Link to="/plateforme">Future plateforme <ArrowUpRight className="ml-1 inline h-3 w-3" /></Link><Link to="/mentions-legales">Mentions légales</Link><Link to="/contact">Contact</Link></div><p className="eyebrow mt-7">Espace Client</p><div className="mt-4 grid gap-3 text-sm"><Link to="/connexion-client">Se connecter · Créer un compte gratuit</Link><span className="text-xs leading-5 text-primary-foreground/50">Pour les particuliers et organisations qui commandent des prestations.</span></div><p className="eyebrow mt-7">Réseau de partenaires</p><div className="mt-4 grid gap-3 text-sm"><Link to="/rejoindre">Rejoindre le réseau professionnel FORMA</Link><span className="text-xs leading-5 text-primary-foreground/50">Artisans, indépendants, PME et entreprises spécialisées, sous coordination FORMA.</span></div></div>
      </div>
